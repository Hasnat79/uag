...
if __name__ == "__main__":
    llama3 = Llama3Loader()
    if os.path.exists(videollama2_text_rep_x_ssbd_dataset_path):
        with open(videollama2_text_rep_x_ssbd_dataset_path) as f:
    ...
    results = {}
    if os.path.exists(llama3_pred_x_videollama2_text_rep_x_ssbd_dataset_path):
      with open(llama3_pred_x_videollama2_text_rep_x_ssbd_dataset_path, 'r') as f:
          results = json.load(f)
...
#             content = f"""The following are the description of activities from frames extracted at 1 frame per second from a video clip:
#             {video_text_rep}
# The activity [A person is {category} with {intensity} intensity] has occurred in the video clip.Find the start and end time of the activity. Provide your best guess by providing the start and end time in json format
# # """         
...
"""
            try : 
              ...
              with open(llama3_pred_x_videollama2_text_rep_x_ssbd_dataset_path, 'w') as f:
                json.dump(results, f, indent=4)
                print(f"Results saved successfully in {llama3_pred_x_videollama2_text_rep_x_ssbd_dataset_path} with {len(results)} samples")
