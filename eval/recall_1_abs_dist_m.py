...
def calculate_iou(gt_start,gt_end,pred_start,pred_end):
    ...
    return iou
def calculate_abs_start_dist(gt_start,gt_end , pred_start, pred_end):
...
def run_eval_on_ssbd(model_name, result_path):
    print(f"\n\n============== {model_name} ==============")
    ...
    for threshold in IoU_threshold:
        ...
        for video_id,video_info in result_null_fix.items():
            ...
            abs_distance_between_gt_pred = calculate_abs_start_dist(gt_start,gt_end,pred_start,pred_end)
            if abs_distance_between_gt_pred <=  original_distance+ threshold*original_distance:
                ...
                correct_count += 1
        print(f"correct_count: {correct_count} len(result): {len(ssbd_result)}")
        recall_1_abs_dist_m = 100*(correct_count / len(ssbd_result))
        print(f"Threshold m = {threshold} R@1 abs dist: {recall_1_abs_dist_m:.2f}")
    ...
    for threshold in abs_distance_threshold:
        ...
        for video_id,video_info in result_null_fix.items():
            ...
            if abs_distance_between_gt_pred <=  original_distance+ threshold*original_distance:
                # print(f"abs_start_distance: {abs_start_distance} gt_start: {gt_start} threshold: {gt_start+gt_start*threshold}")
                correct_count += 1
        print(f"correct_count: {correct_count} len(result): {len(result)}")
        recall_1_abs_dist_m = 100*(correct_count / len(result))
        print(f"Threshold m = {threshold} R@1 abs dist: {recall_1_abs_dist_m:.2f}")
